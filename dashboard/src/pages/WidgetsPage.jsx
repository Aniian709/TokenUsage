...
function useOverlayController() {
  ...
  const saveConfig = async (next) => {
    ...
    setSaving(true);
    try {
      setConfig(await saveOverlayConfig(next));
    } finally {
      setSaving(false);
    }
  };

  return { config, saving, saveConfig };
}
...
function MenuBarDisplayCard({ overlayConfig, overlaySaving, onSaveOverlayConfig }) {
  ...
  const selectedIds = useMemo(
    () =>
      normalizeMenuBarItems(
        available ? settings?.menuBarItems : localMenuBar.items,
        availableItems,
        maxItems,
      ),
    [available, availableItems, localMenuBar.items, maxItems, settings?.menuBarItems],
  );
  const slotIds = useMemo(() => fillTwoSlots(selectedIds, availableItems), [availableItems, selectedIds]);
  const showStats = available ? settings?.showStats !== false : localMenuBar.showStats !== false;
  const animatedIcon = available
  ...
  const saveSelection = (ids) => {
    ...
    if (available) {
      setSetting("menuBarItems", normalized);
      return;
    }
    ...
    <article className="rounded-xl border border-oai-gray-200 bg-white p-5 transition-colors duration-200 dark:border-oai-gray-800 dark:bg-oai-gray-900 sm:p-6">
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2 sm:gap-4">
        {slotConfigs.map(({ slot, currentValue, options }) => (
          <MenuBarSlotSelect
            ...
            value={currentValue}
            options={options}
            disabled={overlaySaving || (!available && !overlayConfig) || !showStats}
            onChange={changeSlot}
          />
        ))}
      </div>
      ...
      <div className="mt-5 divide-y divide-oai-gray-100 border-t border-oai-gray-100 dark:divide-oai-gray-800 dark:border-oai-gray-800">
        <MenuBarToggleRow
          ...
          onChange={() => {
            ...
            onSaveOverlayConfig({
              ...overlayConfig,
              windows: {
                ...overlayConfig.windows,
                menubar: {
                  ...overlayConfig.windows.menubar,
                  enabled: !overlayConfig.windows.menubar?.enabled,
                },
              },
            });
          }}
        ...
        <MenuBarToggleRow
          ...
          onChange={() => {
            if (available) {
              setSetting("showStats", !showStats);
              return;
            }
            if (!overlayConfig) return;
            onSaveOverlayConfig({
        ...
        <MenuBarToggleRow
          ...
          onChange={() => {
            if (available) {
              setSetting("animatedIcon", !animatedIcon);
              return;
            }
            if (!overlayConfig) return;
            onSaveOverlayConfig({
...
function HeaderCta() {
  ...
  if (platform === "mac-web") {
    return (
      <a
        href="https://github.com/Aniian709/TokenUsage/releases/latest"
        target="_blank"
...
function DesktopOverlayCard({ config, saving, onSaveOverlayConfig }) {
  const toggleWidget = async (widgetId) => {
    ...
    const next = {
      ...
      windows: {
        ...config.windows,
        [widgetId]: {
          ...config.windows[widgetId],
          enabled: !config.windows[widgetId]?.enabled,
        },
      },
    };
    await onSaveOverlayConfig(next);
  };
  ...
  const widgets = [
    { id: "summary", label: copy("widgets.summary.name") },
    { id: "heatmap", label: copy("widgets.heatmap.name") },
    { id: "topModels", label: copy("widgets.topModels.name") },
    { id: "limits", label: copy("widgets.limits.name") },
  ];

  return (
    <article className="rounded-xl border border-oai-gray-200 bg-white p-5 dark:border-oai-gray-800 dark:bg-oai-gray-900 sm:p-6">
      <div className="flex items-start gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-oai-gray-100 dark:bg-oai-gray-800">
          <Pin className="h-5 w-5 text-oai-black dark:text-white" aria-hidden="true" />
        </div>
        <div className="min-w-0">
          <h3 className="text-lg font-semibold text-oai-black dark:text-white">
            Windows Desktop Widgets
          </h3>
          <p className="mt-1 text-sm leading-relaxed text-oai-gray-500 dark:text-oai-gray-400">
            Toggle always-on-top desktop cards that mirror the macOS widgets and stay synced with your local dashboard data.
          </p>
        </div>
      </div>
      ...
      <div className="mt-5 grid grid-cols-1 gap-3 sm:grid-cols-2">
        {widgets.map((widget) => (
          <div
            key={widget.id}
            className="flex items-center justify-between rounded-xl border border-oai-gray-200 bg-oai-gray-50 px-4 py-3 dark:border-oai-gray-800 dark:bg-oai-gray-950/60"
          >
            <div>
              <p className="text-sm font-medium text-oai-black dark:text-white">{widget.label}</p>
              <p className="mt-0.5 text-xs text-oai-gray-500 dark:text-oai-gray-400">
                264 × 124 desktop card
              </p>
            </div>
            <ToggleSwitch
              checked={Boolean(config?.windows?.[widget.id]?.enabled)}
      ...
      <div className="mt-5 rounded-xl border border-oai-gray-200 bg-oai-gray-50 p-4 dark:border-oai-gray-800 dark:bg-oai-gray-950/60">
        <div className="mb-3 flex items-center gap-2">
          <SlidersHorizontal className="h-4 w-4 text-oai-gray-500 dark:text-oai-gray-400" aria-hidden="true" />
          <p className="text-sm font-medium text-oai-black dark:text-white">Desktop Appearance</p>
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <label className="flex flex-col gap-2">
            <span className="text-xs font-semibold uppercase tracking-[0.08em] text-oai-gray-500 dark:text-oai-gray-400">
              Scale
            </span>
            <input
              type="range"
        ...
        <div className="mt-4">
          <MenuBarToggleRow
            label="Click Through"
            hint="When enabled, the widget windows stay visible but won’t intercept mouse clicks."
            checked={Boolean(config?.appearance?.clickThrough)}
            disabled={!config || saving}
            onChange={() => setAppearance({ clickThrough: !config?.appearance?.clickThrough })}
          />
        </div>
      </div>
    </article>
...
export function WidgetsPage() {
  const { config, saving, saveConfig } = useOverlayController();
          ...
          <FadeIn y={12} delay={0.06}>
            <section aria-label={copy("widgets.menubar.section.title")} className="mb-12 sm:mb-14">
              ...
              <MenuBarDisplayCard
                ...
                onSaveOverlayConfig={saveConfig}
              />
            </section>
          </FadeIn>

          <FadeIn y={12} delay={0.08}>
            <section aria-label="Windows desktop widgets" className="mb-12 sm:mb-14">
