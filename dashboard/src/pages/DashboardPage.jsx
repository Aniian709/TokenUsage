...
export function DashboardPage({
  ...
  const [firstDetailDay, setFirstDetailDay] = useState(null);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const response = await getUsageDaily({
          baseUrl,
          accessToken,
          from: "2020-01-01",
          to: todayKey || formatDateLocal(new Date()),
          timeZone,
          tzOffsetMinutes,
        });
        if (!active) return;
        const rows = Array.isArray(response?.data) ? response.data : [];
        const first = rows.find((row) => row?.day && !row?.future)?.day || null;
        setFirstDetailDay(first);
      } catch (_err) {
        if (!active) return;
        setFirstDetailDay(null);
      }
    })();
    return () => {
      active = false;
    };
  }, [accessEnabled, accessToken, baseUrl, timeZone, todayKey, tzOffsetMinutes]);
  const range = useMemo(() => {
    if (period === "custom" && customFrom && customTo) {
      return { from: customFrom, to: customTo };
    }
    const base = getRangeForPeriod("day", {
      timeZone,
      offsetMinutes: tzOffsetMinutes,
      now: mockNow,
    });
    const endDate = parseUtcDateKey(base.to || base.from) || new Date();
    if (period === "day") return base;
    if (period === "week") {
      const start = addUtcDays(endDate, -6);
      return { from: formatDateUTC(start), to: base.to };
    }
    if (period === "month") {
    ...
    if (period === "total") {
      return { from: firstDetailDay || base.to, to: base.to };
    }
  ...
  }, [mockNow, period, timeZone, tzOffsetMinutes, customFrom, customTo, firstDetailDay]);
  ...
  const shareDailyToTrend = period === "week" || period === "month" || period === "total";
  const useDailyTrend = period === "week" || period === "month" || period === "total";
  ...
  const trendRowsForDisplay = useMemo(() => {
    if (period === "week") {
      return Array.isArray(visibleDaily)
        ? visibleDaily.filter((row) => row?.day && !row?.future).slice(-7)
        : [];
    }
    if (period === "month") {
      return Array.isArray(visibleDaily)
        ? visibleDaily.filter((row) => row?.day && !row?.future).slice(-30)
        : [];
    }
    ...
    return trendRows;
  }, [daily, period, trendRows, useDailyTrend, visibleDaily]);
  const trendFromForDisplay = useMemo(() => {
    if (period === "week" || period === "month") {
      return trendRowsForDisplay[0]?.day || from;
    }
    return useDailyTrend ? from : trendFrom;
  }, [from, period, trendFrom, trendRowsForDisplay, useDailyTrend]);
  const trendToForDisplay = useMemo(() => {
    if (period === "week" || period === "month") {
      return trendRowsForDisplay[trendRowsForDisplay.length - 1]?.day || to;
    }
    return useDailyTrend ? to : trendTo;
  }, [period, to, trendTo, trendRowsForDisplay, useDailyTrend]);
