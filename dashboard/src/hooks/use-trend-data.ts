...
export function useTrendData({
  ...
  const storageKey = (() => {
    ...
    if (mode === "hourly") {
      ...
      return `tokenusage.trend.${cacheKey}.${host}.hourly.${dayKey}.${tzKey}`;
    }
    if (mode === "monthly") {
      const toKey = to || "today";
      return `tokenusage.trend.${cacheKey}.${host}.monthly.${months}.${toKey}.${tzKey}`;
    }
    const rangeKey = `${from || ""}.${to || ""}`;
    return `tokenusage.trend.${cacheKey}.${host}.daily.${rangeKey}.${tzKey}`;
  })();
