...
const { loadSessionUsageRows } = require("./session-history");
const {
  readOverlayConfig,
  writeOverlayConfig,
  syncOverlayHost,
...
async function scopedUsageRows(queuePath, url) {
  const scope = getRequestedUsageScope(url);
  const sessionRows = await loadSessionUsageRows();
  const allRows = sessionRows.length > 0 ? sessionRows : readQueueData(queuePath);
  return {
    scope,
    allRows,
    rows: filterRowsByUsageScope(allRows, scope),
...
function createLocalApiHandler({ queuePath }) {
  ...
  return async function handleLocalApi(req, res, url) {
    ...
    if (p === "/functions/tokentracker-usage-model-breakdown") {
      ...
      const to = url.searchParams.get("to") || "";
      const timeZoneContext = getTimeZoneContext(url);
      const { rows: scopedRows, scope, excludedSources } = await scopedUsageRows(qp, url);
    ...
    if (p === "/api/widget-overlays") {
      const method = String(req.method || "GET").toUpperCase();

      if (method === "GET") {
        json(res, await readOverlayConfig());
        return true;
      }
      ...
      if (method === "POST") {
        ...
        const body = await readJsonBody(req);
        const nextConfig = normalizeOverlayConfig(body);
        const saved = await writeOverlayConfig(nextConfig);
        const localBaseUrl = `http://${req.headers.host || "127.0.0.1:7680"}`;
        await syncOverlayHost({ baseUrl: localBaseUrl });
      ...
      if (method === "DELETE") {
        ...
        const reset = await writeOverlayConfig(undefined);
        await stopOverlayHost();
        json(res, { ok: true, config: reset });
        return true;
      }
